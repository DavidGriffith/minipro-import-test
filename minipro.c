	...
	for (i = 0; i < 16; i++)
	{
		...
		if(read_buffer[1])
		{
			msg[0] = MP_RESET_PIN_DRIVERS;
				...
				msg_send(handle, msg, 4);
				minipro_close(handle);
			ERROR2("Overcurrent protection detected while testing VPP pin driver %d!\007\n", vpp_pins[i].pin);

		}
		if(!read_buffer[6 + vpp_pins[i].pin])
			error = 1;
		printf("VPP driver pin %d is %s\n", vpp_pins[i].pin, read_buffer[6 + vpp_pins[i].pin]? "OK" : "Bad");
	}
	printf("\n");
				...
				if(read_buffer[1])
					{
						...
						msg_send(handle, msg, 4);
						minipro_close(handle);
						ERROR2("Overcurrent protection detected while testing VCC pin driver %d!\007\n", vcc_pins[i].pin);

					}
				if(!read_buffer[6 + vcc_pins[i].pin])
					error = 1;
				printf("VCC driver pin %d is %s\n", vcc_pins[i].pin, read_buffer[6 + vcc_pins[i].pin]? "OK" : "Bad");
			}
		printf("\n");
				...
				if(read_buffer[1])
					{
						...
						msg_send(handle, msg, 4);
						minipro_close(handle);
						ERROR2("Overcurrent protection detected while testing GND pin driver %d!\007\n", gnd_pins[i].pin);

					}
				if(read_buffer[6 + gnd_pins[i].pin])
					error = 1;
				printf("GND driver pin %d is %s\n", gnd_pins[i].pin, read_buffer[6 + gnd_pins[i].pin]? "Bad" : "OK");
			}
