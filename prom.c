...
static prom_t prom_table[] = {
    ...
    {.gnd_pins = (uint8_t[]){12, 0},
     .vcc_pins = (uint8_t[]){24, 0},
     .data_bus_pins = (uint8_t[]){9, 10, 11, 13, 14, 15, 16, 17, 0},
     .addr_bus_pins = (uint8_t[]){8, 7, 6, 5, 4, 3, 2, 1, 23, 22, 21, 19, 0},
     .ce_lo_pins = (uint8_t[]){20, 0},
    ...
    {.gnd_pins = (uint8_t[]){12, 0},
     .vcc_pins = (uint8_t[]){24, 0},
     .data_bus_pins = (uint8_t[]){9, 10, 11, 13, 14, 15, 16, 17, 0},
     .addr_bus_pins = (uint8_t[]){8, 7, 6, 5, 4, 3, 2, 1, 23, 22, 21, 19, 18, 0},
     .ce_lo_pins = (uint8_t[]){20, 0},
     .compare_mask = 0xff}
};
...
int prom_init(minipro_handle_t *handle) {
  ...
  set_io_pins(zif_dir, prom_table[type].addr_bus_pins, MP_PIN_DIRECTION_OUT,
              pin_count);
  ...
  if (handle->minipro_set_zif_direction(handle, zif_dir)) return EXIT_FAILURE;
  if (handle->minipro_set_zif_state(handle, zif_state)) return EXIT_FAILURE;

  // Set all GND and VCC power pins
  set_pwr_pins(pin_drivers, prom_table[type].gnd_pins, 1, pin_count, GND_PIN);
  set_pwr_pins(pin_drivers, prom_table[type].vcc_pins, 1, pin_count, VCC_PIN);
  ...
  if (handle->minipro_set_pin_drivers(handle, pin_drivers)) return EXIT_FAILURE;
  // Set VPP and VCC voltages
  return handle->minipro_set_voltages(handle, handle->device->voltages.vcc,
...
int prom_read(minipro_handle_t *handle, uint32_t address, uint8_t *buffer,
  ...
  for (int i = 0; i < lenght; i++) {
    ...
    set_bits(zif_state, prom_table[type].addr_bus_pins, address + i, pin_count);
    if (handle->minipro_set_zif_state(handle, zif_state)) return EXIT_FAILURE;

    // Now read the zif pins
    if (handle->minipro_get_zif_state(handle, zif)) return EXIT_FAILURE;

    // Convert zif data bus value and write it to buffer
    buffer[i] = get_bits(zif, prom_table[type].data_bus_pins, pin_count);
  }
