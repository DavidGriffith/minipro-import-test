...
int Config_open(const char *path)
{
	...
	config_content = (struct conf*) realloc(config_content,
			(config_lines_qty + 50) * sizeof(struct conf));
	...
	while (fgets(temp, LINE_LENGTH, pFile))
	{
		if (strlen(temp) == LINE_LENGTH - 1)
		{
			printf("Config error. Line too long: %d\n", counter1);
			ret = 1;
			errno = EINVAL;
			break;
		}
		...
		if (strlen(result) >= sizeof(config_content[counter].param_name))
		{
			printf("Config error. Key too long on line: %d\n", counter1);
			ret = 1;
			errno = EINVAL;
			break;
		}
		...
		if (strlen(result) >= sizeof(config_content[counter].param_value))
		{
			printf("Config error. Value too long on line: %d\n", counter1);
			ret = 1;
			errno = EINVAL;
			break;
		}
