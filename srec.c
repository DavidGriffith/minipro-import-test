...
typedef enum { S0 = 0, S1, S2, S3, S4, S5, S6, S7, S8, S9 } Rectype;
...
static record_t parse_record(uint8_t *record) {
  ...
  uint8_t checksum_c = byte_count + (rec.address >> 24) + (rec.address >> 16) +
                       (rec.address >> 8) + (rec.address & 0xff);
  ...
  uint8_t checksum_f =
      (hex(record[byte_count * 2 + 2]) << 4) | hex(record[byte_count * 2 + 3]);
...
static int write_record(FILE *file, record_t *record) {
  ...
  uint8_t checksum = record->count + 1 + fmt / 2 + (record->address >> 24) +
                     (record->address >> 16) + (record->address >> 8) +
                     (record->address & 0xff);

  fprintf(file, "S%01X%02X%0*X", record->type, record->count + 1 + fmt / 2, fmt,
          record->address);
  while (record->count--) {
        ...
        switch (rec.type) {
          ...
          case S3:
            ...
            if (chip_size >= rec.address + rec.count)
              // copy record data
              memcpy(&(data[rec.address]), rec.data, rec.count);
            else
